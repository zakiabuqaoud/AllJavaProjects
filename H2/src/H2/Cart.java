...
public class Cart extends JFrame implements ActionListener  {
	...
	public Cart(CustomerWindow w) {
		...
		for(int i=0;i<w.cst.cartProducts.size();i++) {
			...
			b[i].setBounds(10, 104, 89, 23);
			b[i].setForeground(Color.WHITE);
			b[i].setBackground(new Color(204, 0, 51));
			b[i].addActionListener(this);
		}
		
			for(int i=0;i<cst.cartProducts.size();++i) {
				pl1[i]=cst.cartProducts.get(i).creatPanel(b[i]);
				pl1[i].setVisible(true);
				p1.add(pl1[i]);
				}
