...
public class ManagerWindow extends JFrame implements ActionListener,ItemListener,ListSelectionListener{
	...
	JTextField t1,t2,t3,t4,t5,t6,productname,price,quantity,id,name,addres,user,pass;
	JTextArea a1,a2,a3,a4;
	
	EStore estore;
	
	JComboBox<Department> department;
...
////*************************************************************************//
	...
	//department
	quantity.setBounds(700, 218,80, 27);
	p2.add(quantity);
	
	Department []q = new Department[estore.depList.size()];
	for(int i=0;i<estore.depList.size();i++) {
		q[i] = estore.depList.get(i);
	}
	department = new JComboBox<Department>(q);
	department.setBounds(700, 274,120, 27);
	p2.add(department);
...
public void valueChanged( ListSelectionEvent e ) {
	...
	if(list2.getSelectedValue()!=null) {
		...
		department.setSelectedItem(p.d);
...
public void actionPerformed(ActionEvent e) {
	...
	if(e.getSource()==b2) {
			...
			for(int i=0;i<estore.depList.size();++i) {
				if((Department)list1.getSelectedValue()==estore.depList.get(i)) {
			estore.depList.get(i).setName(t1.getText());
			estore.depList.get(i).setDescription(a1.getText());
			JOptionPane.showMessageDialog(null, "The Department "+estore.depList.get(i).getName()+" Updated Successfully");
		}
			}
	}
		...
		if(list2.getSelectedValue()!=null) {
			...
			try {
				...
				if(productname.getText().compareTo("")!=0){
					...
					p.d = (Department) department.getSelectedItem();
	...
	if(e.getSource() == b7) {
		Customer c = (Customer) list3.getSelectedValue();
		if(c!=null) {
			if((c.toString().compareTo(user.getText())==0)) {
			...
			else JOptionPane.showMessageDialog(null, "This User Name Already Founded");
		}else JOptionPane.showMessageDialog(null, "Choose ACustomer To Update","",0);
	}
	
	if(e.getSource() == b9) {
		Product p = (Product) list2.getSelectedValue();
		estore.productsList.remove(p);
		model2.removeElement(p);
		Product.plist.remove(p);
		for(int i=0;i<estore.customersList.size();i++) {
		/**/	estore.customersList.get(i).cartProducts.remove(p);
		}
	}
	
	if(e.getSource() == b8) {
		Customer cs = (Customer) list3.getSelectedValue();
		estore.customersList.remove(cs);
		Customer.clist.remove(cs);
		model3.removeElement(cs);
	}
	
	if(e.getSource() == b11) {
		Department dep = (Department) list1.getSelectedValue();
		estore.depList.remove(dep);
		Department.depList.remove(dep);
		model1.removeElement(dep);
	}
